...
The **basepenguins** package provides tools to convert R scripts and R Markdown/Quarto documents (or other specified file types) that use the [**palmerpenguins**](https://allisonhorst.github.io/palmerpenguins/) library to use the versions of the `penguins` and `penguins_raw` datasets now included in R 4.5.0's **datasets** package.
...
1. `convert_files()`: Convert specified files to new output locations
2. `convert_files_inplace()`: Modify files in-place
3. `convert_dir()`: Convert files in a specified directory to a new output directory
4. `convert_dir_inplace()`: Convert files in a directory in-place
...
1. Removes any `library(palmerpenguins)`, `library('palmerpenguins') or `library("palmerpenguins") call
2. Replaces the long variable names with their shorter equivalents:
   - `bill_length_mm` → `bill_len`
   - `bill_depth_mm` → `bill_dep`
   - `flipper_length_mm` → `flipper_len`
   - `body_mass_g` → `body_mass`
3. Replaces `ends_with("_mm")` (a pattern used in `dplyr::select()`) with `starts_with("flipper_"), starts_with("bill_")`
...
We'll use the `"my_examples"` directory that we created above with the call to `example_dir("my_examples")`.
...
potential_files <- files_to_convert("my_examples")
potential_files
```
...
files_to_convert("my_examples", extensions = "R")
```

```{r}
# All extensions
files_to_convert("my_examples", extensions = NULL)
```
...
```{r}
input_files <- files_to_convert("my_examples")
...
```{r}
no_penguins_file <- "my_examples/no_penguins.Rmd"
